...
```
Read .context/goal.md and .context/milestones.md.
...
2. The single next concrete step toward it.
...
```
I think M<N> is done. Read .context/milestones.md and verify against the "Done when" checklist. For each criterion, cite the file/line that proves it (or say "not yet" and what's missing). Do not mark anything done unless every criterion is satisfied.
```
